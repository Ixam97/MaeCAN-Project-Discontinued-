...
namespace CAN_UDP_Monitor
{
    unsafe static class Program
    {
        ...
        static int numKnownCmds = 8;
        static int[] knownCmds ={ 0x00, 0x04, 0x05, 0x06, 0x0b, 0x11, 0x18, 0x1B};
        ...
        //Formatiert das Datagram für die ausgabe aauf der Konsole:
        static string FormatDatagram(byte[] b_datagram, int port)
        {
            StringBuilder s_datagram = new StringBuilder();
            s_datagram.AppendFormat("{0} >> {1:d} {2} :: ID: ", port, DateTime.Now.Date, DateTime.Now.TimeOfDay);
            for(int i = 0; i < 4; i++)
            {
        ...
        static void canDecodeBuilder(bool cmdResp, int cmdIndex, byte[] datagram)
        {
            ...
            {
                string state;
                if (datagram[9] == 0) state = "rot"; else state = "grün";

                Console.Write("Zubehör {0}, Stellung: {1}", getAcc(datagram), state);
            }
