...
throttle_slider.on('input', function(){
  ...
  if (speedupdate){ 
    ...
    throttle_slider_handle.textContent = Math.ceil((this.value / 10)*2.5) + ' km/h';
...
throttle_slider_handle.textContent = Math.ceil((throttle_slider[0].value / 10)*2.5) + ' km/h';
...
function updateSpeed(value){ 
    if (value == 'error') {
      ...
      throttle_slider_handle.textContent = Math.ceil((value / 10)*2.5) + ' km/h';
    }
}
